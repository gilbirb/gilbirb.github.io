...
One or more threads/processes never get access to the resource they need, even though the system as a whole is making progress. Someone is always being skipped over. Example is question number 7
...
This is **livelock**. Both processes are doing work (constantly seek-ing), but are not progressing. The fix is to make the seek and read one atomic operation. Acquire lock before seek, and release lock after read.